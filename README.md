...
A comprehensive dashboard has been developed using Metabase with Docker deployment:

#### Database Setup
- **Data Export**: CSV data exported to Supabase database
- **Connection**: Metabase connected to Supabase for real-time data access
- **Docker Deployment**: Containerized Metabase instance for easy deployment
- **Database File**: Metabase configuration included in repository (`metabase.db.mv`)
...
- **Correlation Analysis**: Visual representation of key factors affecting student outcomes
...
The prediction application provides an intuitive interface for dropout risk assessment.

#### Live Deployment
🚀 **Access the live application**: [Student Performance Prediction App](https://student-performance-prediction-aji.streamlit.app/)
...
- **Real-time Results**: Instant predictions using the trained Random Forest model
...
#### Usage Instructions
1. Navigate to the web interface (local or deployed)
...
Through extensive correlation analysis and data exploration, we identified the most significant factors contributing to student dropout. The top features with highest correlation to student outcomes include:

**Top Contributing Factors (in order of correlation strength):**
1. **Curricular Units (2nd Semester) - Approved**: Strong positive correlation with graduation
2. **Curricular Units (1st Semester) - Approved**: Academic performance in first semester
3. **Curricular Units (2nd Semester) - Grade**: Academic achievement levels
4. **Curricular Units (1st Semester) - Grade**: Foundation academic performance
5. **Scholarship Holder**: Financial support impact on completion
6. **Tuition Fees Up to Date**: Financial commitment and stability
7. **Application Mode**: Method of enrollment and student commitment
8. **Previous Qualification (Grade)**: Prior academic background
9. **Age at Enrollment**: Student maturity and readiness factors
10. **Educational Special Needs**: Support requirements for completion

These factors demonstrate that **academic performance** (particularly in the first two semesters), **financial stability** (scholarships and fee payment), and **student background characteristics** are the primary predictors of student success or dropout risk.
